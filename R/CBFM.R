...
#' @param offset A matrix of offset terms to applied in association with the `formula` argument.
#' @param weights A matrix or vector of weights to be applied in association with the `y` argument, potentially used to control the contribution of the responses to the log likelihood. If a vector is supplied, then it must be of length `nrow(y)`, and this it assumed the same weights applies to all species. Note weights are not internally scaled in any way.
#' @param ncores To speed up fitting, parallelization can be performed, in which case this argument can be used to supply the number of cores to use in the parallelization. Defaults to `detectCores()-1`.
...
#' @param trial_size Trial sizes to use for binomial distribution. This can either equal a scalar or a matrix with the same dimension as `y`.
#' @param dofit Should the CBFM be fitted? If set to `FALSE`, then the function terminates (and return nothing) immediately after copying the C++ file to the compilation directory; please see the `TMB_directories` argument below.
#' @param stderrors Should standard errors of the estimates be calculated? This defaults to `TRUE`, but can be set of `FALSE` if only point estimations of the regression coefficients for the covariates and basis functions are desired. Please see details later on for more information on how standard errors are constructed.
#' @param select For cases where `formula` involves smoothing terms, setting this to `TRUE` adds an extra penalty to each smoothing term so it can be penalized to zero i.e., null space penalization. Please see [mgcv::gam.selection()] and [mgcv::step.gam()] for more details. Note also this argument has no effect on any parametric terms in the model i.e., it can not shrink parametric terms to zero.
#' @param gamma For cases where `formula` involves smoothing terms, setting this to a value greater than one leads to smoother terms i.e., increased penalization. Note the argument can either be set to a scalar, or a vector with length equal to the number of species i.e., `ncol(y)`. This argument plays exactly the same role as the `gamma` argument in [mgcv::gam()]. Finally, note this argument has no effect on any parametric terms or the basis functions part of the CBFM.
#' @param knots For cases where `formula` involves smoothing terms, this is an optional list containing user specified knot values to be used for basis construction. For most bases, the user simply supplies the knots to be used, which must match up with the k value supplied. Please see [mgcv::gam()] for more details. Note the knot values are assumed to be common across species.
#' @param ziselect For cases where `ziformula` involves smoothing terms, setting this to `TRUE` adds an extra penalty to each smoothing term so it can be penalized to zero i.e., null space penalization. Please see [mgcv::gam.selection()] and [mgcv::step.gam()] for more details. Note also this argument has no effect on any parametric terms in the model i.e., it can not shrink parametric terms to zero.
#' @param zigamma For cases where `ziformula` involves smoothing terms, setting this to a value greater than one leads to smoother terms i.e., increased penalization. Note the argument can either be set to a scalar, or a vector with length equal to the number of species i.e., `ncol(y)`. This argument plays exactly the same role as the `gamma` argument in [mgcv::gam()]. Finally, note this argument has no effect on any parametric terms or the basis functions part of the CBFM.
#' @param ziknots For cases where `ziformula` involves smoothing terms, this is an optional list containing user specified knot values to be used for basis construction. For most bases, the user simply supplies the knots to be used, which must match up with the k value supplied. Please see [mgcv::gam()] for more details. Note the knot values are assumed to be common across species.
...
#' \item{basis_effect_mat: }{A matrix of starting values for the species-specific regression coefficients related to the combined matrix of basis functions. Again, the number of rows is equal to the number of species, while the number of columns should equal to `ncol(B_space, B_time, B_spacetime)` (or whatever the supplied basis functions are).}
...
#' }
#' @param TMB_directories A list with two elements, identifying the directory where TMB C++ file exists (`cpp`), and the directory where the corresponding compiled files to be placed (`compile`). Unless you really want to do some real mucking around, these should be left at their default i.e., the directory where the packages were installed locally. Please note a version of the C++ file will be copied to the `compile` directory.
...
#' \item{inner_maxit: }{The maximum number of iterations for the inner (EM) algorithm. Defaults to 1, although it is recommended this is tested out and increased if convergence presents as an issue.}
...
#' \item{initial_betas_dampen: }{A dampening factor which can be used to reduce the magnitudes of the starting values obtained for the species-specific regression coefficients corresponding to the model matrix i.e., `betas`. To elaborate, when starting values are not supplied as part of `start_params`, the function will attempt to obtain starting values based on fitting a stacked species distribution model. While this generally works OK, sometimes it can lead to bad starting values for the `betas`, due to the stacked species distribution model being overfitted or too unstable. An *ad-hoc* fix to this is to dampen/shrink these initial values to be closer to zero, thus allowing the PQL estimation algorithm to actually "work". For instance, setting `initial_betas_dampen = 0.8` set the magnitudes of the staring values for all the `betas` to 0.8 of their original values, including the species-specific intercepts.}

#' \item{subsequent_betas_dampen: }{A dampening factor which can be used to reduce the magnitudes of the values obtained for the species-specific regression coefficients corresponding to the model matrix i.e., `betas`, during the running of the PQL estimation algorithm. To elaborate, during the PQL algorithm updates are made to the regression coefficients related to the combined matrix of basis functions, conditional on the regression coefficients corresponding to the model matrix. However, sometimes this updating can fails due to the latter being overfitted, say. If this occurs, then an *ad-hoc* attempt is made where the update is done conditional on a dampened/shrunk set of the regression coefficients corresponding to the model matrix. For instance, setting `subsequent_betas_dampen = 0.25` sets the magnitudes of all the regression coefficients related to the model matrix to a quarter of their original size, including the species-specific intercepts.
#' Note this argument *only* comes into play when the first attempt, which can be thought of as updating with `subsequent_betas_dampen = 1`, to update the regression coefficients associated with the combined matrix of basis functions fails. }

#' \item{initial_dispparam_dampen: }{Similar to `initial_betas_dampen`, a dampening factor which can be used to reduce the magnitudes of the starting values obtained for the species-specific dispersion parameters.}

#' \item{subsequent_dispparam_dampen: }{Similar to `subsequent_dispparam_dampen`, a dampening factor which can be used to reduce the magnitudes of the values obtained for the species-specific disperson parameters, during the running of the PQL estimation algorithm.}

#' \item{gam_method: }{When smoothing terms are included in the model, this controls the smoothing parameter estimation method. Defaults to "REML", which is maximum restricted likelihood estimation. However other options are available; please see the `method` argument in [mgcv::gam()] for the available options. In fact, note [mgcv::gam()] defaults to using "GCV.Cp", which is based on generalized cross-validation. This is generally faster, but can be slightly more unstable, and hence why restricted maximum likelihood estimation is adopted as the default. }
...
#' \item{seed: }{The seed to use for the PQL algorithm. This is only applicable when the starting values are randomly generated, which only occurs for a subset of parameters even when starting parameters are not supplied and `CBFM` tries to generate its own starting values.}

#' \item{initial_ridge: }{A ridge parameter that can be included to act as a ridge penalty when using CBFM's default approach to constructing initial/starting values of the regression coefficients related to the covariates. This can either be a scalar or a vector equal to the number of columns in the model matrix implied by `formula` and `data` arguments. *Currently, no check is made to ensure the length is compatible with this!*}

#' \item{ridge: }{A ridge parameter that can be included to act as a ridge penalty when estimating the regression coefficients related to the covariates. This can either be a scalar or a vector equal to the number of columns in the model matrix implied by `formula` and `data` arguments. *Currently, no check is made to ensure the length is compatible with this!*}

#' \item{initial_ziridge: }{A ridge parameter that can be included to act as a ridge penalty when using CBFM's default approach to constructing initial/starting values of the regression coefficients related to the covariates for modeling the zero-inflation probabilities. This can either be a scalar or a vector equal to the number of columns in the model matrix implied by `ziformula` and `data` arguments. *Currently, no check is made to ensure the length is compatible with this!*}

#' \item{ziridge: }{A ridge parameter that can be included to act as a ridge penalty when estimating the regression coefficients related to the covariates for modeling the zero-inflation probabilities. This can either be a scalar or a vector equal to the number of columns in the model matrix implied by `ziformula` and `data` arguments. *Currently, no check is made to ensure the length is compatible with this!*}

#' \item{trace: }{If set to `TRUE` or `1`, then information at each iteration step of the outer algorithm will be printed. }
#' }
...
#' \describe{
#' \item{rank: }{The rank of the community-level covariance matrices of the basis function regression coefficients. This either equals to a single scalar/character string equal to "full", or a vector or scalars/character strings (equal to "full") with length equal to how many of `B_space/B_time/B_spacetime` are supplied. If it is a single scalar or character string, then it is assumed the same rank is used for all the community-level covariance matrices.
#' 
#' The rank/s should be at least 2, although internal checks are also performed to assess if the rank is too large such that estimation is not feasible. If the character string "full" is used, then a full-rank covariance matrix is estimated.} 
...
#' 
#' The rank/s should be at least 2, although internal checks are also performed to assess if the rank is too large such that estimation is not feasible. If the character string "full" is used, then a full-rank correlation matrix is estimated. 
#' 
#' Finally. note if a particular element in `G_control$structure` is set to "identity" or "homogeneous", then a corresponding element in `G_control$rank` must still be supplied, but it is ignored.}

#' \item{structure: }{The structure to assume for the between-species correlation matrix if it is estimated. This either equals to a single character string or a vector of character strings with length equal to how many of `B_space/B_time/B_spacetime` are supplied. If it is a single string, then it is assumed the same form is used for all the correlation matrices. The current options for each element are "unstructured" (default) which assumes an unstructured form subject to the corresponding element in `G_control$rank`, "identity" which assumes an unknown, estimated scalar multiplied by an identity matrix, and "homogeneous" which is the same as assuming an unstructured form but the entire matrix is the multiplied by an unknown, estimated scalar.
...
#'
#' @details 
#' Community-level basis function models (CBFMs, Hui et al., 2023) are a class of joint species distribution models for spatio-temporal multivariate abundance data, which builds on the ideas of fixed rank kriging (FRK, Cressie and Johannesson, 2008; Zammit-Mangion and Cressie, 2017) and multivariate spatio-temporal mixed models (e.g., Bradley et al., 2018) and adapts them specifically for spatio-temporal multivariate abundance data in community ecology. CBFMs are a (closely connected) alternative to the popular spatio-temporal latent variable models (LVMs) approach for joint species distribution modeling, as available in a number of packages such as [Hmsc::Hmsc-package()] (Tikhonov et al., 2020), [gllvm::gllvm()] (Niku et al., 2019), and [boral::boral()] (Hui, 2016); see also Warton et al., (2015a,b), Thorson et al. (2016) and Ovaskainen and Abrego (2020) among others for general introductions to the use of LVMs in community ecology.  The key difference between LVMs and CBFMs is that rather than using a small number of latent variables (which are assumed to be random across observational units), CBFMs use a larger number of pre-specified spatial and/or temporally-indexed basis functions which remain fixed throughout the model fitting process (Hefley et al., 2017; Cressie et al., 2021). The randomness instead comes from species-specific regression coefficients related to these basis functions, which in turn induce spatio-temporal correlations within and between-species. 
#' 
#' In using a basis function approach, CBFMs can thus be framed as a type of generalized additive model (GAM, Guisan et al., 2002; Wood, 2017) . This in turn means CBFMs can leverage from the plethora of techniques which have been already developed for GAMs, with one notable benefit being that computationally, CBFMs tend to more efficient and scale better than many existing implementations of spatio-temporal LVMs (at least, at the time of writing; see also Hui et al., 2023).
#' 
#'
#' ## Some mathematics
...
#' 
#' The vector of predictors \eqn{x_i} is created based on the `formula` and `data` arguments. Smoothing terms are permitted in `formula`, and these can be included in the same way as in [mgcv::gam.models()]; see also [mgcv::smooth.terms()]. Note smoothing terms in this context also permits the inclusion of (species-specific) random intercepts and slopes, through the use of the `s(..., bs = "re")`; please see [mgcv::random.effects()] and [mgcv::gam.vcomp()] for more details. These may be used, say, as a simple approach to account for nested sampling designs, multiple data sources/surveys etc..., although please note these random effects are specific to each species i.e., they are *not* random row effects as found in packages such as [boral::boral()] and [gllvm::gllvm()], and also are currently are not designed to draw the slopes from a common distribution as in [Hmsc::Hmsc-package()] or Pollock et al., (2014), say.
#' 
#' When smoothing terms are included in the CBFM, a check of the smooth basis dimension and whether it is adequate is also automatically performed, courtesy of the [mgcv::k.check()] function; see [mgcv::k.check()] and [mgcv::choose.k()] for more general details. Furthermore, selection of smoothing terms as well as user specified knot values for basis construction are also possible; please see [mgcv::gam()], [mgcv::gam.selection()], and [mgcv::step.gam()] for more details. However, we warn the user **some of the smoothers available as part of `mgcv` e.g., [mgcv::linear.functional.terms()], has not been fully tested for CBFM**, so some make may not work. If you encounter any problems, please post a Github issue on the CBFM repository!
#' 
#' Next, the vector basis functions \eqn{b_i} is formed from the `B_space`, `B_time` and `B_spacetime` arguments. At least one of these arguments must be supplied. As an example, suppose we wish to fit a CBFM with spatial and temporal basis functions included in an additive manner. Then only `B_space` and `B_time` should be supplied, in which case the mean regression model for the CBFM can be written as:
#' 
#' \deqn{g(\mu_{ij}) = \eta_{ij} = x_i^\top\beta_j + b_{i,space}^\top a_{j,space} + b_{i,time}^\top a_{j,time},}
#'
#' where \eqn{b_i = (b_{i,space}, b_{i,time})} and \eqn{a_j = (a_{j,space}, a_{j,time})}. If purely spatial or temporal multivariate abundance data is recorded, then one should only supply `B_space` and `B_time`, respectively, and the above mean regression model is simplified accordingly.
#' 
#' As another example, suppose we wish to include spatio-temporal basis functions (which are formed from a tensor-product). Then only `B_spacetime` should be supplied, in which case the mean regression model for the CBFM can be written as:
#' 
...
#' 
#' Note for zero-inflated distributions, it is the mean of the non-zero inflated component which is modeled and *not the mean of the entire distribution.*
#' 
#' 
#' ** Remark on flavors and choices of CBFMs:** 
#' For spatio-temporal multivariate abundance data, we have found the above two examples are usually the two most appropriate "flavors" CBFM to apply, although the precise form should of course depend upon the precise interpretation and question/s of interest (Hui et al., 2023). As seen above, we may want the separate sets of spatial and temporal basis functions to be included in an additive manner (this is analogous to an LVM where separate spatial LVs and temporal LVMs are added together), or have a single set of spatio-temporal basis functions formed from a tensor-product say (this is analogous to an LVM with one set of spatio-temporal LVs), or have a combination of the two where (say) the basis functions included in `B_space` and `B_time` are accounting for correlations on a coarse scale while the basis functions included in `B_spacetime` are accounting for resolutions on a fine scale. We refer the interested reader to Thorson et al., (2016) and Thorson (2019) for examples of similar kinds of constructs and flavors within the LVM framework.
#' 
# Also, this package only implements one possible version of a wider class of CBFMs; other potentially superior versions e.g., spatial basis functions with temporally varying corresponding regressions coefficients, are possible under the CBFM framework, but are outside the scope of this package (sorry!).      
#'    
#' In principle, it is possible to employ a more data-driven approach such as cross-validation or information criteria to choose which type of and/or the number of basis functions to include in the arguments `B_space/B_time/B_spacetime`. This similar to choosing the number of latent variables in a LVM. We refer the reader to [mgcv::choose.k()] as some of the advice provided there may be applicable to CBFMs e.g., using residual analysis to informally check whether an increase the number of spatial and/or temporal basis functions is required, and echo a sentiment written there (while acknowledging things are more tricky with spatial and/or temporal basis functions, and for discrete responses!):
#' 
...
#' 
#' where \eqn{G} is a so-called baseline between-species correlation matrix, \eqn{\Sigma} is the community-level covariance matrix for the basis function regression coefficients, and \eqn{kronecker(\cdot)} is the Kronecker product operator. When multiple sets of basis functions are included, then this carries over. For instance, in the example above involving a CBFM with separate spatial and temporal basis functions, with only `B_space` and `B_time` supplied, we have
#'   
...
#' }
#' 
#' Missing (`NA`) values are permitted as part of the response matrix. These are simply passed over during the fitting process i.e., effectively the action [stats::na.omit()] is taken.
#'
#' 
#' 
...
#' 
#' At the same time, it would be remiss not to provide some brief recommendations based on previous experience, and we do so below. Please see also Hui et al., (2023) and the examples later on for some more concrete applications.
#' \describe{
#' \item{`B_space`: }{We have found resolution adaptive thin-plate spline basis functions (Tzeng and Huang, 2018), as implemented in [autoFRK::mrts()], work fairly well here as spatial basis functions, particularly in terms of interpolation. They are simple to use and require the user to only supply the number of basis functions, which itself is tied to the resolution at which the user wants to model their spatial correlations.
#' 
...
#' \item{`B_time`: }{Both of the approaches mentioned above for `B_space` can also be applied here, although with temporal basis functions we have generally found the approach implemented in [FRK::auto_basis()] to work satisfactorily in many cases, given their customizability and sparsity (local support).}

#' \item{`B_spacetime`: }{A general starting point for constructing spatio-temporal basis functions is to make use of a tensor-product form (analogous to [mgcv::te()]). That is, after constructing a set of spatial and a set of temporal basis functions, we can use the [tensorproduct()] function to construct the tensor-product and include them `B_spacetime`.
#' 
#' It is recommended both "ingredient" basis functions used in the tensor-product are sparse in design to facilitate computation e.g., as implemented in [FRK::auto_basis()]; see the `FRK` package as well as Wilke et al. (2019) for some examples. Also, we recommend you do not use these same "ingredient" basis functions in the `B_space` and `B_time` arguments, as this may lead to overfitting. Put another way, and as hinted at previously, if `B_spacetime` is supplied at the same time as either `B_space` and/or `B_time` is supplied, then they should generally be constructed to model different resolutions/scales of the spatio-temporal correlation.}
#' }
#' 
#' 
...
#' 
#' If `start_params` is not supplied, then CBFM attempts to obtain starting values based on fitting an appropriate stacked species distribution model. This generally works alright, but can sometimes fail badly e.g., if the stacked species distribution model severely overfits for one or more species. A tell-tale sign of when it occurs is if from the returned CBFM fit, the estimates of regression coefficients corresponding to the spatial and/or temporal basis functions i.e., `basis_effects_mat`, are extremely close to zero for these problematic species. There are not always easy fixes for such situations (as it may reflect an underlying, perhaps intriguing feature of the proposed model for the predictors, data, or it may genuinely be the stacked species distribution model is already fitting incredibly well!). Two *ad-hoc* fixes are available through `control$initial_betas_dampen/control$initial_dispparam_dampen` and `control$initial_ridge` arguments, but it is not guaranteed to work.
#' 
...
#' \item{y, data, trial_size, weights: }{The supplied response matrix, covariate information data frame, trial size(s), and weights.}
...
#' \item{B: }{The full matrix basis functions i.e., basically the result of `cbind(B_space, B_time, B_spacetime)`.}

#' \item{which_B_used: }{A vector of length three, indicating which of `B_space, B_time, B_spacetime` was supplied. For example `which_B_bused = c(1,0,0)` implies only `B_space` was supplied.}

#' \item{num_B_space: }{The number of spatial basis functions supplied i.e., `ncol(B_space)`.}

#' \item{num_B_time: }{The number of temporal basis functions supplied i.e., `ncol(B_time)`.}

#' \item{num_B_spacetime: }{The number of spatio-temporal basis functions supplied i.e., `ncol(B_spacetime)`.}

#' \item{num_B: }{The total number of basis functions supplied i.e., `ncol(cbind(B_space, B_time, B_spacetime))`.}

#' \item{converged: }{Indicates whether or not the PQL estimation algorithm converged. Note results may still be outputted (and sensible?!) even if this is `FALSE`.}
...
#' \item{deviance_explained: }{The *percentage* of null deviance explained by the model. In community ecology this is typically not very high; please see [varpart()] for more capacity to perform variance partitioning in a CBFM. This is constructed by examining `deviance` to `null_deviance`.}

#' \item{deviance_explained_perspecies: }{The *percentage* of null deviance explained by the model, on a per-species basis. This is constructed by examining `deviance_perspecies` to `null_deviance_perspecies`.}

#' \item{pql_logLik: }{The value of the PQL i.e., the likelihood plus the quadratic penalty term, upon convergence.}

#' \item{edf/edf1: }{A matrix of estimated degrees of freedom for each model parameter in `formula`. The number of columns of the matrix should be equal to the number of species i.e., `ncol(y)`. Penalization means many of these are less than one. `edf1` is an alternative estimate of EDF. Note these values are pulled straight from the GAM part of the estimation algorithm, and consequently may only be *very* approximate. }

#' \item{ziedf/ziedf1: }{A matrix of estimated degrees of freedom for each model parameter in `ziformula`. The number of columns of the matrix should be equal to the number of species i.e., `ncol(y)`. Penalization means many of these are less than one. `edf1` is an alternative estimate of EDF. Note these values are pulled straight from the GAM part of the estimation algorithm, and consequently may only be *very* approximate. }

#' \item{pen_edf: }{A list with each element containing a vector of the estimated degrees of freedom associated with each smoothing term in `formula`. The length of the list should be equal to the number of species i.e., `ncol(y)`. Note these values are pulled straight from the GAM part of the estimation algorithm, and consequently may only be *very* approximate.}

#' \item{zipen_edf: }{A list with each element containing a vector of the estimated degrees of freedom associated with each smoothing term in `ziformula`. The length of the list should be equal to the number of species i.e., `ncol(y)`. Note these values are pulled straight from the GAM part of the estimation algorithm, and consequently may only be *very* approximate.}

#' \item{k_check: }{A list resulting from the application of [mgcv::k.check()], used as a diagnostic test of whether the smooth basis dimension is adequate for smoothing terms included in `formula`, on a per-species basis. Please see [mgcv::k.check()] for more details on the test and the output. Note if no smoothing terms are included in `formula`, then this will be a list of `NULL` elements.}

#' \item{vcomp: }{A list with length equal to `ncol(y)`, where each element contains a vector of the estimated variance components (as standard deviations) associated with the smoothing terms included in `formula`. This output is only really useful when one or more of the smoothing terms were included in the CBFM as species-specific intercepts/slopes (see [mgcv::random.effects()] for more details), in which case the corresponding values in `vcomp` are the estimated variance components (estimated standard deviations to be precise) associated with these random effects; see [mgcv::random.effects()] and [mgcv::gam.vcomp()] for more details on the one-to-one relationship between smoothing parameters in GAMs and variance components in mixed models. Note if no smoothing terms are included in `formula`, then this will be a list of `NULL` elements.}

#' \item{betas: }{The estimated matrix of species-specific regression coefficients corresponding to the model matrix created. The number of rows in `betas` is equal to the number of species i.e., `ncol(y)`.}

#' \item{zibetas: }{The estimated matrix of species-specific regression coefficients corresponding to the model matrix created for the zero-inflation component. The number of rows in `zibetas` is equal to the number of species i.e., `ncol(y)`. Recall a logit scale to model the probability of zero-inflation i.e., \eqn{log(\pi/(1-\pi))} is regressed against covariates.}

#' \item{basis_effects_mat: }{The estimated matrix of species-specific regression coefficients corresponding to the combined matrix of basis functions. The number of rows in `basis_effects_mat` is equal to the number of species i.e., `ncol(y)`.}
...
#' \item{Sigma_space/Loading_Sigma_space/nugget_Sigma_space: }{The estimated community-level covariance matrix/loadings/nugget effect associated with the spatial basis functions, if `B_space` is supplied. Note if `Sigma_control$custom_space` was supplied, then `Sigma_space` would be directly the supplied matrix, while the loading and nugget arguments are set to `NULL`.}

#' \item{G_space/Loading_G_space/nugget_G_space: }{The estimated baseline between-species correlation matrix/loadings/nugget effect associated with the spatial basis functions, if `B_space` is supplied. Note if `Sigma_control$custom_space` was supplied, then a covariance matrix is estimated instead. Note if `G_control$custom_space` was supplied, then `G_space` would be directly the supplied matrix, while the loading and nugget arguments are set to `NULL`.}

#' \item{Sigma_time/Loading_Sigma_time/nugget_Sigma_time: }{The estimated community-level covariance matrix/loadings/nugget effect associated with the temporal basis functions, if `B_time` is supplied. Note if `Sigma_control$custom_time` was supplied, then `Sigma_time` would be directly the supplied matrix, while the loading and nugget arguments are set to `NULL`.}

#' \item{G_time/Loading_G_time/nugget_G_time: }{The estimated baseline between-species correlation matrix/loadings/nugget effect associated with the temporal basis functions, if `B_time` is supplied. Note if `Sigma_control$custom_time` was supplied, then a covariance matrix is estimated instead. Note if `G_control$custom_time` was supplied, then `G_time` would be directly the supplied matrix, while the loading and nugget arguments are set to `NULL`.}

#' \item{Sigma_spacetime/Loading_Sigma_spacetime/nugget_Sigma_spacetime: }{The estimated community-level covariance matrix/loadings/nugget effect associated with the spatio-temporal basis functions, if `B_spacetime` is supplied. Note if `Sigma_control$custom_spacetime` was supplied, then `Sigma_spacetime` would be directly the supplied matrix, while the loading and nugget arguments are set to `NULL`.}

#' \item{G_spacetime/Loading_G_spacetime/nugget_G_spacetime: }{The estimated baseline between-species correlation matrix/loadings/nugget effect associated with the spatio-temporal basis functions, if `B_spacetime` is supplied. Note if `Sigma_control$custom_spacetime` was supplied, then a covariance matrix is estimated instead. Note if `G_control$custom_spacetime` was supplied, then `G_spacetime` would be directly the supplied matrix, while the loading and nugget arguments are set to `NULL`.}

#' \item{stderrors: }{The supplied argument for `stderrors` i.e., whether standard errors were calculated.}

#' \item{covar_components: }{If `stderrors = TRUE`, then a list containing with the following components:
#' 1) `topleft`, which is a matrix corresponding to the top-left block of the full Bayesian posterior covariance matrix. The top-left block specifically relates to the regression coefficients associated with the measured predictors i.e., the covariance matrix associated with `object$betas`, along with `object$zibetas` if zero-inflated distributions are used;
#' 2) `topright`, which is a matrix of the top-right block of the full Bayesian posterior covariance matrix. The top-right block specifically relates to the cross-covariance of the regression coefficients associated with the measured predictors (plus the coefficients associated with the probabilities of zero-inflation) and the basis functions i.e., the cross-covariance matrix between `object$betas` and `object$basis_effects_mat`;
#' 3) `bottomright`, which is a matrix containing components of the bottom-right block of the full Bayesian posterior covariance matrix. The bottom-left block specifically relates to the regression coefficients associated with the basis functions i.e., the covariance matrix associated with `object$basis_effects_mat`.
#' 
...
#' 
#' @details # Warning
#' 1. CBFMs are designed for \emph{spatio-temporal} multivariate abundance data, such that you can sensibly construct basis functions from the space-time coordinate of each observational unit (Hui et al., 2023). **Please do not use them for data which are not spatially or temporally indexed**. We recommend you fit standard LVMs in those scenarios e.g., as available in [gllvm::gllvm()], [boral::boral()], and [Hmsc::sampleMcmc()].
#' 
#' 2. Not for some distributions it is not the mean of the entire distribution which is modeled. For example, in zero-inflated distributions it is the mean of the non-zero-inflated component which is modeled with the regression model described above. In zero-truncated distributions, it is the mean of the base count distribution which is modeled with the regression model described above.
#' 
#' 3. Not all (in fact, not many) of the smoothing terms available in [mgcv::gam.models()] have been fully tested out, so please be aware some make not work well, if at all! 
#' 
...
#' 
#' As with any real-life statistical modeling problem, it is almost impossible to determine what the source of the issue is without looking at the data and specific application first hand. Therefore, we can only provide some general avenues to pursue below as a first step towards making CBFM run on your data, and of course we can not guarantee the output produced from this debugging makes any ecological sense!
#' 
#' * An initial thing to try is to bump up the number of inner iterations i.e., `control$inner_maxit}`from the default of 1 to something like 10, 20 or even higher. This will give more opportunities for the inner estimation component of the PQL algorithm (where all regression and smoothing coefficients are updated) to try converge to a stable point, before proceeding to estimating the other model parameters.
#' 
#' * Sometimes the starting values CBFM constructs are not great! A common situation where this occurs is when smoothers are employed in `formula` and the data are (extremely) overdispersed or show signs of complete or quasi-separation. A simple way to try and break out of bad automated starting values is to make use of the `control$initial_betas_dampen/control$initial_dispparam_dampen` arguments or `control$initial_ridge`, both of which effectively dampen the starting estimated coefficients from potentially extreme magnitudes and can facilitate the underlying PQL estimation algorithm to "get going".
#' 
#' * Alternatively, supplying your own "wisely chosen" starting values is never a bad thing, plus it can often help to speed up the fitting process. In our experience, often a decent way to obtain starting values is to fit stacked GAMs using [mgcv::gam()] with the same formula as you will use in `formula`, plus smoothing terms to account for space and/or time. Some template code is provided as follows:
#' ```
...
#' 
#' # Fit CBFM 
#' # Note also Hmsc generates and fits models assuming a probit link, 
...
#' # Note we assume each site is only visited once, but the code below can be adapted to 
...
#' dat_train$time <- factor(dat_train$time) # Re-factor so it now only has five time points
...
#' # This is partly not surprising given we used a more approximate fitting method for 
...
CBFM <- function(y, formula, ziformula = NULL, data, 
                 B_space = NULL, B_time = NULL, B_spacetime = NULL, 
                 offset = NULL, weights = NULL, ncores = NULL, 
                 family = stats::gaussian(), trial_size = 1, 
                 dofit = TRUE, stderrors = TRUE, 
                 ...
                 start_params = list(betas = NULL, zibetas = NULL, basis_effects_mat = NULL, 
                                     dispparam = NULL, powerparam = NULL, 
                                     custom_space_lambdas = NULL, custom_time_lambdas = NULL, custom_spacetime_lambdas = NULL),
                 TMB_directories = list(cpp = system.file("executables", package = "CBFM"), compile = system.file("executables", package = "CBFM")),
                 control = list(maxit = 100, inner_maxit = 1, 
     ...
     ## Validate and set up observation weights
     if(!is.null(weights)) {
          if(is.vector(weights)) {
               if(length(weights) != num_units)
                    stop("weights vector must have length equal to nrow(y).")
               weights <- matrix(weights, nrow = num_units, ncol = num_spp)
               }
               if(is.matrix(weights)) {
                    if(!identical(dim(weights), dim(y)))
                         stop("weights matrix must have the same dimensions as y.")
                    }
               if(any(weights < 0, na.rm = TRUE))
                    stop("weights must be non-negative.")
          }
     if(is.null(weights))
          weights <- matrix(1, nrow = num_units, ncol = num_spp)
     ...
     initfit_fn <- function(j, formula, control) {
          ...
          if(family$family == "zipoisson") {
               ...
               ziHmat <- diag(control$initial_ziridge+1e-15, nrow = ncol(ziX))
               ...
               if(all(control$initial_ridge == 0))
                    fit0 <- gam(tmp_formula, 
                           data = data.frame(response = y[,j], data), 
                           weights = weights[,j] * (1-w), 
                           offset = cw_offset, 
                           knots = knots, 
                           method = control$gam_method, 
                           family = "poisson", 
               ...
               if(all(control$initial_ziridge == 0))
                    fitzi <- suppressWarnings(gam(tmp_ziformula, 
                                                  data = data.frame(taus = w, data), 
                                                  weights = weights[,j],
                                                  knots = ziknots, 
                                                  method = control$gam_method, 
                                                  family = "binomial", 
          ...
          if(family$family %in% c("ztpoisson")) {
               ...
               if(is.null(cw_offset))
                    cw_offset <- numeric(num_units)
               tmp_dat <- data.frame(response = c(y[,j], numeric(10)), 
                                     data[c(1:nrow(data),1:10),], 
                                     off = c(cw_offset, numeric(10)),
                                     obs_wt = c(weights[,j], rep(0, 10))) # Append some zeros speeds ziplss up a heck of a lot!
               ...
               if(!inherits(fit0, "try-error")) {
                    ...
                    cw_eta <- MM %*% fit0$coefficients
                    if(!is.null(model.offset(model.frame(fit0))))
                         cw_eta <- cw_eta + model.offset(model.frame(fit0))[1:(nrow(MM))]
                    fit0$logLik <- weights[,j] * .dztpois(y[,j], lambda = exp(cw_eta), log = TRUE) # .dztpois y = 0 values to -Inf, and handles NA values; because offset is in formula, then offset is contained in linear.predictors
     ...
     while(diff > control$tol & counter <= control$maxit) {
          ...
          while(inner_err > 1e-3 & inner_counter <= control$inner_maxit) {
               ...
               update_basiscoefsspp_fn <- function(j) {
                    ...
                    tidbits_data$obs_weights <- as.vector(weights[,j])
                    ...
                    CBFM_objs <- TMB::MakeADFun(data = tidbits_data, 
                                                parameters = tidbits_parameters, 
                                                DLL = getDLL, 
                                                hessian = FALSE, 
                                                silent = TRUE)
                    
                    new_fit_CBFM <- try(nlminb(start = CBFM_objs$par, 
                                               objective = CBFM_objs$fn, 
               ...
               update_Xcoefsspp_fn <- function(j) {
                    ...
                    if(family$family %in% c("zipoisson")) { # M-step
                         ...
                         if(!all(control$ridge == 0))
                              fit0 <- gam(tmp_formula, 
                                          ...
                                          knots = knots, 
                                          method = control$gam_method, 
                                          weights = weights[,j] * (1-getweights[,j]), 
                                          H = Hmat, 
                                          family = "poisson", 
                                          select = select, 
                                          gamma = full_gamma[j])
                         ...
                         if(!all(control$ziridge == 0))
                              fitzi <- suppressWarnings(gam(tmp_ziformula, 
                                                            data = data.frame(taus = getweights[,j], data), 
                                                            weights = weights[,j],
                                                            knots = ziknots, 
                                                            method = control$gam_method, 
                                                            H = ziHmat, 
                    ...
                    if(family$family %in% c("ztpoisson")) {
                         ...
                         tmp_dat <- data.frame(response = c(y[,j], numeric(10)), 
                                               data[c(1:nrow(data), 1:10),], 
                                               off = c(new_offset, numeric(10)),
                                               obs_wt = c(weights[,j], rep(0, 10))) # Append some zeros avoids a non-convergence problem in mgcv
                         tmp_formula <- as.formula(paste("response", paste(as.character(formula), collapse = " "), "+ offset(off)" ) )
                         if(!all(control$ridge == 0))
                              fit0 <- gam(list(tmp_formula, ~1), 
                                          data = tmp_dat, 
                         ...
                         fit0$linear.predictors <- X %*% fit0$coefficients + new_offset + formula_offset
                         fit0$logLik <- weights[,j] * .dztpois(y[,j], lambda = exp(fit0$linear.predictors), log = TRUE) # .dztpois sets y = 0 values to -Inf, and handles NA values
                         fit0$logLik <- sum(fit0$logLik[is.finite(fit0$logLik)])                               
                         }
               ...
               if(which_B_used[j1]) {
                    if(is.null(G_control[[ three_custom_options[j1] ]])) {
                         ...
                         new_G_current <- .update_G_fn(Ginv = get(three_LoadingnuggetG_options[j1])$invcov, 
                                                       ...
                                                       weights = weights, # Note this is passed as matrix
     ...
     if(family$family == "ztpoisson") {
          ...
          out_CBFM$fitted[out_CBFM$fitted < 1 | out_CBFM$fitted == Inf] <- 1 # Predictions less than 1 or at infinity will be almost certainly due to the linear predictor being so close to zero you get underflow issues...unless the linear predictor is stupidly large
     ...
     if(family$family == "ztnegative.binomial") {
          ...
          out_CBFM$fitted[out_CBFM$fitted < 1 | out_CBFM$fitted == Inf] <- 1 # Predictions less than 1 will be almost certainly due to the linear predictor being so close to zero you get underflow issues...unless the linear predictor is stupidly large
          out_CBFM$linear_predictors[which(out_CBFM$y == 0)] <- NA
          out_CBFM$fitted[which(out_CBFM$y == 0)] <- NA
          }
     ...
     if(stderrors) {          
          ...
          if(family$family[1] %in% c("zipoisson","zinegative.binomial")) {
               ...
               weights_mat_betabeta <- weights_mat_betabeta * weights # Apply observation weights
               weights_mat$out_zeroinflzeroinfl[is.na(out_CBFM$y)] <- 0
               weights_mat$out_zeroinflbetas[is.na(out_CBFM$y)] <- 0
               weights_mat$out_zeroinflzeroinfl <- weights_mat$out_zeroinflzeroinfl * weights
               weights_mat$out_zeroinflbetas <- weights_mat$out_zeroinflbetas * weights
               }
